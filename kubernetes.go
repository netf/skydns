...
func (ksync *KubernetesSync) OnUpdate(services []api.Service) {
	...
	for _, service := range services {
		...
		if exists && (info.portalPort != service.Spec.Ports[0].Port || !info.portalIP.Equal(serviceIP)) {
			err := ksync.removeDNS(name, info)
			if err != nil {
				log.Printf("failed to remove dns for %q: %s\n", name, err)
			}
		}
		log.Printf("adding new service %q at %s:%d/%s (local :%d)\n", name, serviceIP, service.Spec.Ports[0].Port, service.Spec.Ports[0].Protocol, service.Spec.Ports[0].Port)
		si := &serviceInfo{
		...
		si.portalPort = service.Spec.Ports[0].Port
